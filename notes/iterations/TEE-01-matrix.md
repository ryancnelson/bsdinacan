...
- `BSIZE` chunking and `do...while` write retry logic dictate that single-fd write failures drop the *unwritten remainder of the current read chunk* for that fd; bytes already accepted by earlier short writes remain, but do not remove the fd from the `head` linked list. Subsequent chunks retry writing to the faulty fd.
