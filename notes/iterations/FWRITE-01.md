...
- Verified `INT64_MAX` limit and `EIO` mapping on zero progress or an oversized return from the callback; positive short writes are retried.
...
- **Guest Capture Wrapper**: Wrote `libc_fwrite_wrapper_probe.c` to spawn a child with duplicated internal test streams and directly verify null byte boundaries over `fwrite`. The wrapper itself outputs `PASS\n` which `CB_MAC_CASE` expects.
