...
- Feature commit `28b76864a1147c68ca8715d22966e0972e61f092`: Woodpecker
  pipeline #64 passed Linux, mac68k, and mac-automation workflows.
- Combined integration commit: Woodpecker checks pending; feature-commit
  checks are not evidence for the combined tree.
...
The combined tree preserves the existing API tail in the order
`getopt_state_location`, `truncate`, `ftruncate`, then appends `getprogname`.
The libc startup validation requires all four operations plus `exit`.
All exit, getopt, stdio, truncate, and errx probes remain linked and invoked.
The FS-01 optional-node-table guards and focused truncate selection are retained.
No guest acceptance is implied by resolving or committing this merge.
