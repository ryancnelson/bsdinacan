...
- `libgen.h`: `dirname` and `basename`, importing NetBSD's pinned
  ...
  task-local call copies the imported routine's own shared, process-wide
  static result into a task-owned buffer -- a separate buffer for each
  of the two functions, so a `basename` call can never invalidate an
  already-returned `dirname` result in the same task, or vice versa --
  before returning, so one task's call can never be silently overwritten
  by another's. The copy is exactly what the import itself already
  computed, so nothing is truncated beyond what NetBSD's own
  `PATH_MAX`-bounded static would already have truncated. A call on an
  old or otherwise incompatible runtime table reports `ENOSYS`.
