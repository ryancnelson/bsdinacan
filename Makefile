...
$(CP_COMMAND_OBJECT): upstream/netbsd/bin/cp/cp.c upstream/netbsd/bin/cp/extern.h \
		include/cannedbsd/abi.h include/cannedbsd/libc.h \
		libc/include/sys/cdefs.h compat/netbsd/include/sys/param.h \
		libc/include/sys/stat.h compat/netbsd/include/assert.h \
		libc/include/err.h libc/include/errno.h libc/include/fcntl.h \
...
$(CP_UTILS_OBJECT): upstream/netbsd/bin/cp/utils.c upstream/netbsd/bin/cp/extern.h \
		include/cannedbsd/abi.h include/cannedbsd/libc.h \
		libc/include/sys/cdefs.h libc/include/sys/mman.h \
		compat/netbsd/include/sys/param.h libc/include/sys/stat.h \
		libc/include/sys/time.h libc/include/sys/extattr.h \
		libc/include/err.h libc/include/errno.h libc/include/fcntl.h \
		libc/include/fts.h libc/include/stdio.h libc/include/stdlib.h \
		libc/include/string.h libc/include/unistd.h | $(BUILD)
	$(CC) $(CPPFLAGS) -Icompat/netbsd/include -Ilibc/include -Iupstream/netbsd/bin/cp $(CFLAGS) \
		-DSMALL \
...
STAT_OLDTABLE_TEST_OBJECT := $(BUILD)/libc_stat_oldtable_probe.o

# FS-STAT-01: proves translate_stat's struct_size guard, not just that
# the new fields exist.
$(STAT_OLDTABLE_TEST_OBJECT): tests/libc_stat_oldtable_probe.c \
		include/cannedbsd/abi.h include/cannedbsd/libc.h \
		libc/include/sys/stat.h | $(BUILD)
	$(CC) $(CPPFLAGS) -Ilibc/include $(CFLAGS) -Dmain=cb_stat_oldtable_main \
		-c tests/libc_stat_oldtable_probe.c -o $@
...
analyze:
	...
	$(CC) $(CPPFLAGS) -Icompat/netbsd/include -Ilibc/include -Iupstream/netbsd/bin/cp -Dmain=cb_cp_main \
		-DSMALL -std=c99 -Wall -Wextra -Werror -Wpedantic \
		-fanalyzer -fsyntax-only upstream/netbsd/bin/cp/cp.c
	$(CC) $(CPPFLAGS) -Icompat/netbsd/include -Ilibc/include -Iupstream/netbsd/bin/cp \
		-DSMALL -std=c99 -Wall -Wextra -Werror -Wpedantic \
		-fanalyzer -fsyntax-only upstream/netbsd/bin/cp/utils.c
	...
	$(CC) $(CPPFLAGS) -Ilibc/include -Dmain=cb_stat_oldtable_main \
		-std=c99 -Wall -Wextra -Werror -Wpedantic \
		-fanalyzer -fsyntax-only tests/libc_stat_oldtable_probe.c
