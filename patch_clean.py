...
# Strip out everything we added incorrectly
text = re.sub(r'static int test_vfs_executable_main.*?\n}\n', '', text, flags=re.DOTALL)
text = re.sub(r'static const struct cb_program_v1 test_vfs_executable_prog.*?\n};\n', '', text, flags=re.DOTALL)
text = re.sub(r'static void test_vfs_executable_nodes.*?\n}\n', '', text, flags=re.DOTALL)
text = text.replace("    test_vfs_executable_nodes();\n", "")
